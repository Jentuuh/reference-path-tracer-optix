    ...
    extern "C" __global__ void __raygen__renderFrame__cell__gathering()
    {
        ...
        for (int u = startU; u < endU; u++)
        {
            for (int v = startV; v < endV; v++)
            {
                ...
                glm::vec3 rayDestination = { probePos.x, probePos.y, probePos.z };
                ...
                // Check if the current probe is facing the light source, if not, skip it
                ...
                {
                    //printf("Contribution! (probe %f %f %f, face %d) \n ", probePos.x, probePos.y, probePos.z, faceIndex);
