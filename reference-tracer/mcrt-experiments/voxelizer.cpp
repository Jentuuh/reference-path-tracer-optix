...
namespace mcrt {
	...
	// Plane-triangle intersection code written by Tomas Akenine-M¨oller (Fast 3D Triangle-Box Overlap Testing, ACM SIGGRAPH courses, 2005). 
	...
	void Voxelizer::voxelize()
	{
		...
		for (auto& v : bbVoxelGrid)
		{
			...
			for (auto& t : voxelizedObject->model->mesh->indices)
			{
				...
				if (triangleBoxOverlap(glm::value_ptr(boxCenter), glm::value_ptr(boxHalfSize), triangleVerts))
				{
					// Add logical voxel
	...
	bool Voxelizer::planeBoxOverlap(float normal[3], float d, float maxbox[3])
	{
		int q;
		float vmin[3], vmax[3];
		for (q = X; q <= Z; q++)
	...
	bool Voxelizer::triangleBoxOverlap(float boxcenter[3], float boxhalfsize[3], float** triverts)
	{
