...
namespace mcrt {
    ...
    extern "C" __global__ void __raygen__renderFrame__cell__scattering()
    {
        ...
        glm::vec3 ogProbe0 = { cellCenter.x - 0.5f * cellSize, cellCenter.y - 0.5f * cellSize, cellCenter.z - 0.5f * cellSize };
        glm::vec3 ogProbe1 = { cellCenter.x + 0.5f * cellSize, cellCenter.y - 0.5f * cellSize, cellCenter.z - 0.5f * cellSize };
        glm::vec3 ogProbe2 = { cellCenter.x - 0.5f * cellSize, cellCenter.y + 0.5f * cellSize, cellCenter.z - 0.5f * cellSize };
        glm::vec3 ogProbe3 = { cellCenter.x + 0.5f * cellSize, cellCenter.y + 0.5f * cellSize, cellCenter.z - 0.5f * cellSize };
        glm::vec3 ogProbe4 = { cellCenter.x - 0.5f * cellSize, cellCenter.y - 0.5f * cellSize, cellCenter.z + 0.5f * cellSize };
        glm::vec3 ogProbe5 = { cellCenter.x + 0.5f * cellSize, cellCenter.y - 0.5f * cellSize, cellCenter.z + 0.5f * cellSize };
        glm::vec3 ogProbe6 = { cellCenter.x - 0.5f * cellSize, cellCenter.y + 0.5f * cellSize, cellCenter.z + 0.5f * cellSize };
        glm::vec3 ogProbe7 = { cellCenter.x + 0.5f * cellSize, cellCenter.y + 0.5f * cellSize, cellCenter.z + 0.5f * cellSize };
        ...
        int probeOffsets[8] = {((cellCoords.z * probeResWidth * probeResHeight) + (cellCoords.y * probeResWidth) + cellCoords.x) * 6 * (optixLaunchParams.cubeMapResolution * optixLaunchParams.cubeMapResolution),
                                ...
                                ((cellCoords.z * probeResWidth * probeResHeight) + ((cellCoords.y + 1) * probeResWidth) + (cellCoords.x + 1)) * 6 * (optixLaunchParams.cubeMapResolution * optixLaunchParams.cubeMapResolution),
                                ...
                                (((cellCoords.z + 1) * probeResWidth * probeResHeight) + (cellCoords.y * probeResWidth) + (cellCoords.x + 1)) * 6 * (optixLaunchParams.cubeMapResolution * optixLaunchParams.cubeMapResolution),
                                (((cellCoords.z + 1) * probeResWidth * probeResHeight) + ((cellCoords.y + 1) * probeResWidth) + cellCoords.x) * 6 * (optixLaunchParams.cubeMapResolution * optixLaunchParams.cubeMapResolution),
                                (((cellCoords.z + 1) * probeResWidth * probeResHeight) + ((cellCoords.y + 1) * probeResWidth) + (cellCoords.x + 1)) * 6 * (optixLaunchParams.cubeMapResolution * optixLaunchParams.cubeMapResolution),
                                };
        ...
        // Find most nearby probe
        for (int p = 0; p < 8; p++)
        {
            glm::vec3 probeDistance = UVWorldPos - cellProbeOrigins[p];
            if (glm::length(probeDistance) < minDistance)
            {
                minDistance = glm::length(probeDistance);
                minDistanceIndex = p;
            }
        }
                    ...
                    for (int stratifyIndexY = 0; stratifyIndexY < STRATIFY_RES_Y; stratifyIndexY++)
                    {
                        ...
                        for (int sample = 0; sample < NUM_SAMPLES_PER_STRATIFY_CELL; sample++)
                        {
                            ...
                            if (distanceToUVSquared < prd.distanceToClosestIntersectionSquared)
                            {
                                ...
                                // ==================================================================================
                                // Sample all probes of this cell
                                // ==================================================================================
                                for (int p = 0; p < 8; p++)
                                {
